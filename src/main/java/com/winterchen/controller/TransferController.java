...
import com.winterchen.model.pay.JsonResults;
import com.winterchen.model.pay.ResponseData;
import com.winterchen.util.pay.*;
...
public class TransferController {
    ...
    private static final String APP_ID = "wx864093399739afc9";

    private static final String MCH_ID = "1516510551";

    private static final String API_SECRET = "K460584casasasasasasasasas123456";
        ...
        if (CollectionUtil.isNotEmpty(restmap) && "SUCCESS".equals(restmap.get("result_code"))) {
            ...
            WebUtil.response(response,
                    WebUtil.packJsonp(callback,
                            JSON.toJSONString(new JsonResults(1, "转账成功", new ResponseData(null, transferMap)),
            ...
            WebUtil.response(response, WebUtil.packJsonp(callback, JSON
                    .toJSONString(new JsonResults(-1, "转账失败", new ResponseData()), SerializerFeatureUtil.FEATURES)));
        }
    }
    ...
    public void orderPayQuery(HttpServletRequest request, HttpServletResponse response, String tradeno,
        ...
        if (StringUtil.isEmpty(tradeno)) {
            WebUtil.response(response, WebUtil.packJsonp(callback, JSON
                    .toJSONString(new JsonResults(-1, "转账订单号不能为空", new ResponseData()), SerializerFeatureUtil.FEATURES)));
        }
        ...
        if (CollectionUtil.isNotEmpty(restmap) && "SUCCESS".equals(restmap.get("result_code"))) {
            ...
            WebUtil.response(response, WebUtil.packJsonp(callback, JSON
                    .toJSONString(new JsonResults(1, "订单转账成功", new ResponseData(null, transferMap)), SerializerFeatureUtil.FEATURES)));
        }else {
            if (CollectionUtil.isNotEmpty(restmap)) {
                LOG.info("订单转账失败：" + restmap.get("err_code") + ":" + restmap.get("err_code_des"));
            }
            WebUtil.response(response, WebUtil.packJsonp(callback, JSON
                    .toJSONString(new JsonResults(-1, "订单转账失败", new ResponseData()), SerializerFeatureUtil.FEATURES)));
        }
    }
